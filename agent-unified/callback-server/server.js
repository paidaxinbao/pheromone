...
async function processWithOpenClaw(incomingMessage) {
  ...
  const prompt = [
    ...
    'If you need to send replies, specify in JSON format:',
    '{',
    '  "outgoing_messages": [',
    '    {',
    '      "recipient": "agent-id",',
    ...
    '}',
    '',
    'If no reply needed, return: { "outgoing_messages": [] }',
    '',
  ...
  try {
    ...
    // Parse response - expect JSON in the output
    ...
    if (jsonMatch) {
      try {
        ...
        console.log(`[${AGENT_ID}] Failed to parse JSON from OpenClaw response`);
      }
    }
      ...
      outgoing_messages: [
        {
          ...
          payload: {
            ...
            content: `Received your message. Processing...`,
          },
        },
      ],
    };
  } catch (error) {
    console.error(`[${AGENT_ID}] OpenClaw error: ${error.message}`);
